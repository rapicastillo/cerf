$jq(function($) {
	//process each square
		var $this = $(this);
 		...
 		var _primaryCountry = $target_form.find("input[name=primary_country]").val(), 
 			...
 			_summShowThumb = $target_form.find("input[name=sing_show_thumb]").val(),
 			_summShowCount =$target_form.find("input[name=sing_show_count]").val()
 			
 			;
 		
 			
 			var blurb_show = $target_form.find("input[name=sing_show_blurb]").val();
 	...
 	*/	
 		...
 		$.ajax({
 			...
 			success: function(d) {
 				...
 				$(d.data.list).each(function(i, item) {
 					...
 					var _previewFilename = item.fields.image ? item.fields.image.url : (item.fields.file ? item.fields.file[0].preview : "/images/ocha.jpg");
 					...
 					var $new_item = $("<div />").addClass("rwapi-filter-item")
						.append($("<h4 />")
							...
							.append(parseInt(_summShowThumb) ? $("<img />").attr("src",  _previewFilename).addClass("rwapi-item-sq-img") : "")
							.append($("<span />").addClass("rwapi-filter-art-spi"));
 					
 					$("#rwapi-filter-items").append($new_item);
 					
 					if (blurb_show) {
 						var _actualBlurb = item.fields.body ? (item.fields.body).replace(/<.*?>/g, '') : "";
 						$new_item.find(".rwapi-filter-art-spi").html(_actualBlurb);
				...
				if (_offset > 0) {
					$("#rwapi-filter-pagination").append(
						$("<div />").addClass("pagination-item ").append(
							$("<a />").addClass("reliefwebapi-submit-form").attr("href", "?offset=0").text("First").attr("offset", 0)
						)
					);
 		
			 		$("#rwapi-filter-pagination").append(
						$("<div />").addClass("pagination-item ").append(
							$("<a />").addClass("reliefwebapi-submit-form").attr("href", "?offset=" + (_offset-1)).text("Previous").attr("offset", _offset-1)
						)
					);
				}
				...
				while (_startingPage * _limit < _total && _startingPage - _offset < 5) {
					$("#rwapi-filter-pagination").append(
						$("<div />").addClass("pagination-item  " + (_startingPage == _offset ? "current-item" : "")).append(
							$("<a />").addClass("reliefwebapi-submit-form").attr("href", "?offset=" + (_startingPage++)).text(_startingPage).attr("offset", _startingPage)
						)
					);
				}
				...
				if (_offset * _limit < _total) {
					$("#rwapi-filter-pagination").append(
							$("<div />").addClass("pagination-item ").append(
								$("<a />").addClass("reliefwebapi-submit-form").attr("href", "?offset=" + (_offset+1)).text("Next").attr("offset", _offset+1)
							)
						);
						
					$("#rwapi-filter-pagination").append(
							$("<div />").addClass("pagination-item  ").append(
								$("<a />").addClass("reliefwebapi-submit-form").attr("href", "?offset=" + Math.floor(_total/_limit)).text("Last").attr("offset", Math.floor(_total/_limit))
							)
						);
				} 
 			}, 
 		...
 		/*
	$(".reliefwebapi-submit-form").live("click", function(event) {
		var $parent = $(this).closest("#reliefwebapi-block-items-filter-page");
		var $targetForm = $parent.find("form.rwapi-filter-form");
		
		event.preventDefault();
		$targetForm.find("input[name=offset]").val($(this).attr("offset"));
		
		console.log("CONSOLE ", $targetForm.serialize(), $targetForm.find("input[name=offset]").val());
		
		window.location = "?" + $targetForm.serialize();
	}); */
});
